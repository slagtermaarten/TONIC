...
The arm of patient 33 is unknown to me, she's shown in the 'NA' arm
...
analyze_top_clones <- function(p_var = 'normalized_frequency') {
  ...
  dtf <- controlled_merge(dtf, patient_labels)
  return(dtf)
}
...
facet_var = NULL
new_var_name <- 'Fraction of T cells newly detected\namong top 50%'
p <- ggplot(frac_new_clones,
       aes_string(x = 'clinical_response', y = 'frac_new',
  ...
  ggrepel::geom_label_repel(data = frac_new_clones[clinical_response == 'NR' & frac_new > .3],
...
Interestingly, this variable adds information over the sample clonality measure,
as the two are not correlated. It also more robustly distinguish responders from
non-responders, as shown below.
