...
Imperfect compatibility wrt gene names in expression matrix and in MSigDB gene
sets
...
sf(ggsea_res_c2 <- ggsea_wrapper(gmt[noi]))
...
cors <- lapply(auto_name(c('harm_mean', 'geom_mean', 'PDL1_US')), 
               function(y_var) {
  message('processing: ', y_var)
  corlist <- sapply(1:nrow(RC_f_n), function(idx) {
    vec <- unlist(RC_f_n[idx, ])
    stopifnot(all(names(vec) == fh[, cell_line]))
    cor(fh[, get(y_var)], vec, method = 'spearman', 
        use = 'pairwise.complete.obs')
  })
  cor_dat <- data.table(gene_symbol=rownames(RC_f_n), s.cor=corlist)
