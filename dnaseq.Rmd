...
```{r, warning=FALSE, echo=FALSE}
tp53_positions <- positions[3]
tp53_gene_dat <- combine_all_aberrations(tp53_positions)
```
...
```{r, warning=FALSE, echo=FALSE}
source('R/read_DNASeq.R')
b2m_positions <- positions[1:2]
b2m_gene_dat <- combine_all_aberrations(b2m_positions)
...
# gene_dat <- combine_all_aberrations(c('B2M'))
gene_dat <- combine_all_aberrations(c('MLH1', 'MLH3', 'MSH2', 'MSH3', 'MSH6',
                                      'PMS1', 'PMS2'))
gene_dat[, table(variant_classification)]

unique(gene_dat, by = 'patient')[, table(mutated)]
```
