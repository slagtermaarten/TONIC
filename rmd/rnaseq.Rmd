...
```{r, warning=FALSE, echo=FALSE}
x_var <- 'per_sequence_gc_content'
x_var <- 'overrepresented_sequences'
...
rna_sample_annotation$best_samples <-
...
```{r, warning=FALSE, echo=FALSE}
plots <- lapply(c(fastqc_vars, 'best_samples'), function(x_var) {
  ...
  ggplot(rna_sample_annotation,
         aes_string(x = x_var, y = 'evenness', group = x_var)) +
    geom_boxplot() +
...
plot_panel_layout(plots = plots,
                  filename = 'plots2/fastqc_transcriptome_evenness.pdf',
...
if (F) {
  ...
  entrez_table <-
    getBM(attributes=c('ensembl_gene_id', 'gene_biotype', 'entrezgene',
          ...
          filters = 'hgnc_symbol', values = rownames(tpms_salmon), mart = ensembl)
  sr(entrez_table)
} else {
...
subtype_predictions <- molecular.subtyping(sbt.model = 'pam50',
  ...
  do.mapping = F)
table(subtype_predictions$subtype)
```
