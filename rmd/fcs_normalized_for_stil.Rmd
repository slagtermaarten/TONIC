...
p <- ggplot(data_pi, aes(x = clinical_response, y = FC, fill = clinical_response))
...
p <- p + scale_y_continuous(name = 'median log2FC immune related gene sets')
FC_boxplot <- p
...
plots <-
  list(NA, plot_params(p_dat = hlm, mn = 'post_induction'),
       prep_comp_data(tp1 = 'Baseline', tp2 = 'Post-induction') %>%
           ggplot(aes(x = Baseline, y = FC)) + geom_point() + geom_smooth() +
           ylab('log2FC median of immune related gene sets') +
           xlab('Baseline median of immune related gene sets') + theme(text =
         element_text(size = 8)),
       plot_params(p_dat = hlm_b, mn = 'post_induction_b'),
       FC_boxplot + theme(text = element_text(size = 8)),
       plot_params(p_dat = hlm_b_r, mn = 'post_induction_b_r')
       )
# { align_plots(plotlist = ., 'hv') }

fn <- file.path(img_dir, 'panel_bayesian.pdf')
plot_panel_layout(plots, filename = fn, ncol = 2, labels = tolower(LETTERS),
                  w = 19.5, h = 27.7, fig_padding = .05)
```
