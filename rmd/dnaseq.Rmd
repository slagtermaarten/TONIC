...
if (F) {
  mutation_overview <- map_dfr(unique(patient_labels$patient),
                                  function(patient) {
    message(patient)
  ...
  saveRDS(mutation_overview, file.path(rds_dir, 'mutation_overview.rds'))
} else {
  mutation_overview <- readRDS(file.path(rds_dir, 'mutation_overview.rds'))
}
...
## Prepare oncop_dat
if (F || recompute_all) {
  BRCA_gene_dat <- combine_all_aberrations(
    gene_symbols = c('B2M', 'BRCA1', 'BRCA2', 'POLE'))
  saveRDS(BRCA_gene_dat, file.path(rds_dir, 'BRCA_gene_dat.rds'))
} else {
  BRCA_gene_dat <- readRDS(file.path(rds_dir, 'BRCA_gene_dat.rds'))
}
...
if (F || recompute_all) {
  oncop_dat <- map_dfr(unique(patient_labels$patient), function(patient) {
    message(patient)
  ...
  oncop_dat %<>%
    controlled_merge(patient_labels[timepoint == 'Baseline',
      .(patient, response, brca1_like, ca15_3, s_til, pd_l1_immunoinfiltrate,
  ...
  saveRDS(oncop_dat, file.path(rds_dir, 'oncop_dat.rds'))
  write_tsv(oncop_dat, file.path('~/TONIC/ext', 'oncop_dat.tsv'))
} else {
  oncop_dat <- readRDS(file.path(rds_dir, 'oncop_dat.rds'))
}

# compute_dnaseq_stats('pat_16')
# oncop_dat[patient == 'pat_16']
# setdiff(patient_labels[!is.na(cf_number), unique(patient)], 
#         oncop_dat[, as.character(patient)])
# oncop_dat[patient == 'pat_65']
# patient_labels[timepoint == 'Baseline', pd_l1_immunoinfiltrate, 
#   by = clinical_response]
  ...
  mutation_specific_mut_loads <- 
    purrr::map_dfr(oncop_dat$patient, function(pat) {
      ...
      if (nrow(vcf_fh) == 0) {
        ...
        return(data.frame('patient' = pat, 'N' = 1e-10, 
...
# mutation_specific_mut_loads[
#   variant_classification == 'Frameshift Variant' & patient == 'pat_26']
...
if (exists('rna_patient_ordering'))
...
pdl1_immuno_track <- oncop_dat %>%
  ...
  scale_fill_manual(name = 'immune cell PD-L1',
    values = setNames(tonic_color_palettes[['brcaness']], 
      c('PD-L1 low', 'PD-L1 high', 'NA')))
...
source('R/constants.R')
recurrent_mutations_track <- gen_oncoplot(BRCA_gene_dat)
...
# print(recurrent_mutations_track)
...
plots <- list(mut_load_hist + track_style + 
              ...
              recurrent_mutations_track + track_style +
               theme(axis.text.y = element_text(size = 6)) +
               theme(legend.position = 'none'),
...
# grid.draw(ggpubr::get_legend(recurrent_mutations_track))
...
legends <- plots %>%
  ...
  purrr:::map(function(p) 
              tryCatch(ggpubr::get_legend(p +theme(legend.position = 'right')),
...
onco_corrs <- oncop_dat %>% 
  controlled_merge(patient_labels[!is.na(cf_number) & timepoint == 'Baseline', 
    .(patient, clinical_response, brca1_like, adaptive_t_cells)],
    by_cols = 'patient') %>%
  dplyr::select(-patient) %>%
  { GGally::ggpairs(data = ., 
    upper = list(continuous = wrap('cor', size = 10)), 
    lower = list(continuous = 'smooth')) }
fn <- file.path(img_dir, 'pairwise_oncoplot_vars_lenient_filtering.pdf')
